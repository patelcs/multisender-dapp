    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mb-12">
        ...
        <p className="mt-4 text-lg text-(--muted) max-w-2xl">
          Deep dive into the Sandwich architecture, smart contracts, and technical specifications.
        </p>
      </div>
      ...
      {/* Footer Links */}
      <section className="mt-20 flex flex-wrap justify-center gap-4 border-t border-(--border) pt-12">
        <a
          href={GITHUB_URL}
          target="_blank"
    ...
    <div className="space-y-16">
      ...
      <section>
        ...
        <p className="text-(--muted) leading-relaxed">
          The Sandwich MultiSender is a stateless, non-custodial forwarder contract. 
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {SUPPORTED_CHAINS.map((chain) => {
            ...
            return (
              <div key={chain.id} className="rounded-2xl border border-(--border) bg-(--card) p-5">
                ...
                <code className="block w-full bg-(--input-bg) p-3 rounded-lg text-[10px] font-mono text-(--muted) break-all border border-(--border)">
                  {addr}
                </code>
              </div>
            );
      ...
      <section>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-(--border) bg-(--card) p-6">
            <h3 className="text-sm font-bold uppercase tracking-widest text-(--muted) mb-4">MultiSend Struct</h3>
            <pre className="overflow-x-auto rounded-xl bg-(--input-bg) p-5 text-xs text-blue-400 leading-relaxed border border-(--border)">
{`struct MultiSend {
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            <div key={item.label} className="rounded-xl border border-(--border) bg-(--card) p-4">
              <div className="text-[10px] font-bold uppercase tracking-wider text-(--muted) mb-1">{item.label}</div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="space-y-4">
            <p className="text-(--muted) leading-relaxed text-sm">
              Sandwich follows a &quot;Zero-Server&quot; philosophy. All user data is persisted 
          ...
          <div className="rounded-2xl border border-(--border) bg-(--accent) p-6">
            ...
            <code className="block bg-(--card) p-3 rounded-lg text-[10px] font-mono text-purple-400 border border-(--border)">
{`{
  "version": 1,
  "contacts": [...],
  "tokens": [...],
      ...
      <section>
        ...
        <div className="rounded-2xl border border-(--border) bg-(--card) p-8">
          <p className="text-sm text-(--muted) leading-relaxed mb-6">
            Sandwich uses a custom Wagmi transport layer. It prioritizes user-defined 
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-(--muted)">Connect to your nearest local node.</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <ChevronRight size={14} className="mt-1 text-purple-500" />
              <div>
                ...
                <p className="text-xs text-(--muted)">Bypass public RPC tracking/logging.</p>
              </div>
            </div>
          </div>
        </div>
